    ...
    <div id="main-width">
        ...
        <form class="form-horizontal" action="#" method="post" enctype="multipart/form-data">
                    ...
                    <div style="width: 200%;max-height: 8em;overflow: auto;font-size: 0.9em;">
                        <select name="course_id" class="form-control">
                            @foreach($course_list as $key => $val)
                                <option {{$group->course_id==$val->course_id?'selected':''}} value="{{$val->course_id}}">{{$val->course_name}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
            ...
            <div class="form-group">
                ...
                <div class="col-sm-3">
                    <input type="text" value="@foreach($group->member as $key => $val){{$val->uno}},@endforeach"
                           length="100" style="width:200%" class="form-control " name="member"
